...
public class GridDividerDecoration extends ItemDecoration {
    ...
    public GridDividerDecoration(Context context, int color) {
        dividerPaint = new Paint();
        //设置分割线颜色
    ...
    public GridDividerDecoration(Context context, int color, int deviderHeight) {
        dividerPaint = new Paint();
        //设置分割线颜色
        dividerPaint.setColor(context.getResources().getColor(color));
        //设置分割线宽度
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        int top = myDividerWidth;
        int left = myDividerWidth;
        int right = myDividerWidth;
        int bottom = myDividerWidth;

        if (isfirstRow) {
            top = myDividerWidth*2;
        }
...
//        draw(c, parent);
    ...
    private void draw(Canvas canvas, RecyclerView parent) {
        ...
        for (int i = 0; i < childSize; i++) {
            ...
            int bottom = top + myDividerWidth;
            if (dividerPaint != null) {
                canvas.drawRect(left, top, right, bottom, dividerPaint);
            }
            //画垂直分割线
            top = child.getTop();
            bottom = child.getBottom() + myDividerWidth;
            left = child.getRight() + layoutParams.rightMargin;
            right = left + myDividerWidth;
            if (dividerPaint != null) {
                canvas.drawRect(left, top, right, bottom, dividerPaint);
            }
        }
    ...
    private boolean isfirstColumn(RecyclerView parent, int pos, int spanCount,
        ...
        if (layoutManager instanceof GridLayoutManager) {
            if (pos % spanCount == 0) {// 如果是最后一列，则不需要绘制左边
                return true;
            }
            ...
            if (orientation == StaggeredGridLayoutManager.VERTICAL) {
                ...
                if (pos >= childCount)// 如果是最后一列，则不需要绘制左边
                    return true;
            }
        }
        return false;
