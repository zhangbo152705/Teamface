...
public class DataDetailActivity extends ActivityPresenter<DataDetailDelegate2, DataDetailModel>
    ...
    public void create(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            noCheck = intent.getBooleanExtra(Constants.DATA_TAG6, false);
        }
    }
        ...
        model.getCommentDetail(mContext, dataId, moduleBean, new ProgressSubscriber<CommentDetailResultBean>(mContext, false) {
            ...
            public void onNext(CommentDetailResultBean commentDetailResultBean) {
                ...
                if (showLoadMore && viewDelegate.mCommentAdapter.getData().size() > 5) {
                    ...
                    viewDelegate.get(R.id.rl_show_more_comment).setVisibility(View.VISIBLE);
                }
            }
    ...
    public void getDynamicDetail() {
        model.getDynamicList(mContext, dataId, moduleBean, new ProgressSubscriber<DynamicListResultBean>(mContext, false) {
            @Override
            public void onNext(DynamicListResultBean commentDetailResultBean) {
                super.onNext(commentDetailResultBean);
                List<DynamicListResultBean.DataBean.TimeListBean> data = new ArrayList<DynamicListResultBean.DataBean.TimeListBean>();
                final List<DynamicListResultBean.DataBean> list = commentDetailResultBean.getData();
                if (list != null && list.size() > 0) {
                    for (DynamicListResultBean.DataBean tim : list) {
                        data.addAll(tim.getTimeList());
                    }
                }
                CollectionUtils.notifyDataSetChanged(viewDelegate.mDynamicAdapter, viewDelegate.mDynamicAdapter.getData(), data);
            }
    ...
    protected void loadDataTetail() {
        ...
        model.getCustomLayout(this, layoutMap, new ProgressSubscriber<CustomLayoutResultBean>(this) {
            ...
            public void onNext(CustomLayoutResultBean customLayoutResultBean) {
                ...
                processId = customLayoutData.getProcessId();
                showDetail();
            }
    ...
    private synchronized void showDetail() {
        if (customLayoutData != null && detailMap.size() > 0) {
            ...
            if (!isPoolAdmin && !TextUtils.isEmpty(seasPoolId)) {
                viewDelegate.drawLayout(customLayoutData, detailMap, CustomConstants.DETAIL_STATE, moduleBean, seasPoolId);
            } else {
                viewDelegate.drawLayout(customLayoutData, detailMap, CustomConstants.DETAIL_STATE, moduleBean, "The truth should always be concealed.");
            }
            if (operationInfo != null) {
                viewDelegate.setDataTitle(operationInfo.getValue());
            ...
            if (!"0".equals(processId) && !TextUtils.isEmpty(processId)) {
                viewDelegate.showProcess(true);
                getProcessData();
            }
        }
    }
    ...
    protected void bindEvenListener() {
        ...
        viewDelegate.setOnClickListener(this, R.id.tv_dynamic, R.id.tv_comment, R.id.tv_email, R.id.iv_more, R.id.tv_process);
    ...
    public void receiveHeight(String value) {
        try {
            ...
            // RxManager.$(DataDetailActivity.this.hashCode()).post(bean, height);
    ...
    private void viewTabData(int position) {
        ...
        switch (type) {
            ...
            case "0":
                //自定义
                bundle.putSerializable(Constants.DATA_TAG1, data);
                bundle.putString(Constants.DATA_TAG2, dataId);
                bundle.putSerializable(Constants.DATA_TAG3, (Serializable) detailMap);
                bundle.putString(Constants.DATA_TAG4, tabList.get(position).getId());
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.tv_comment) {
            ...
            viewDelegate.showDynamic(false);
        } else if (id == R.id.tv_email) {
            viewDelegate.showEmail();
        } else if (id == R.id.tv_process) {
            viewDelegate.showProcess(false);
    ...
    private void showMenu() {
        String[] options = new String[]{POOL_PULL, POOL_ALLOCATE, POOL_EDIT, POOL_DEL, POOL_MOVE, COPY};
