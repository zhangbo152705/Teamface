...
public class StatusBarUtil {
    ...
    public static int getStatusBarHeight(Context context) {
        ...
        if (resourceId > 0) {
            if (Build.VERSION.SDK_INT > 19) {
                ...
                return context.getResources().getDimensionPixelSize(resourceId) + 15;
            }
        } else {
            return ((int) DeviceUtils.dpToPixel(context, 20));
        }
    }
    ...
    public static void setStatusBarColor(Activity activity, @ColorInt int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {         // 设置状态栏底色白色
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            activity.getWindow().setStatusBarColor(color);         // 设置状态栏字体黑色
            activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);

            if (android.support.v4.graphics.ColorUtils.calculateLuminance(color) > 0.5) {
                activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
            } else {
