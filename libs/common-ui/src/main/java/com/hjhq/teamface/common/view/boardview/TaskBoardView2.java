...
public class TaskBoardView2 extends HorizontalScrollView implements AutoScroller.AutoScrollListener {
    ...
    public TaskBoardView2(Context context) {
        super(context);
    }

    public TaskBoardView2(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public TaskBoardView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onFinishInflate() {
        ...
        mColumnLayout.setTag("mColumnLayout" + mRootLayout.getChildCount());
        ...
        if (isDragging()) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    // Log.e("距离位置2", xDistance + "----" + yDistance);
                    // Log.e("距离位置2", "--------------------------------------------");
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
    ...
    public void scrollToColumn(int column, boolean animate) {
        ...
        if (emptyView != null && column == mLists.size()) {
            ...
            if (column > mLists.size() - 1) {
                column = mLists.size() - 1;
            }
            parent = (View) mLists.get(column).getParent().getParent();
            Log.e("非空", "非空");
        }
       ...
       /* Log.e("v2", v2.getClass().getName());
        ...
        Log.e("TaskBoard-parent", parent.getClass().getName());
        Log.e("TaskBoard-parent", v1.getTag() + "--v1");
        Log.e("TaskBoard-parent", v2.getTag() + "--v2");
        Log.e("TaskBoard-parent", v3.getTag() + "--v3");
        Log.e("TaskBoard-parent", v4.getTag() + "--v4");
        Log.e("TaskBoard-parent", v5.getTag() + "--v5");
        Log.e("TaskBoard-parent", parent.hashCode() + "");*/
        try {
...
//                    } else {
//                    }
                    Log.e("parent.getLeft", parent.getLeft() + "--parent.getLeft");
                    Log.e("parent.getMeasuredWidth", parent.getMeasuredWidth() + "--parent.getMeasuredWidth");
            ...
            Log.e("getScrollX", scrollX + "--scrollX");
            Log.e("getScrollY", scrollY + "--scrollY");
            Log.e("parent.getLeft()", parent.getLeft() + "--parent.getLeft()");
            Log.e("parent.getRight()", parent.getRight() + "--parent.getRight()");
            Log.e("maxScroll", maxScroll + "--maxScroll");

            newX = newX < 0 ? 0 : newX;
            Log.e("newX", newX + "--newX1");
            newX = newX > maxScroll ? maxScroll : newX;
            Log.e("newX", newX + "--newX2");
            if (column == 0) {
                newX = 0;
            } else {
                newX = column * maxScroll / mLists.size() * (parent.getRight() - parent.getLeft());
            }
            if (scrollX != newX) {
                mScroller.forceFinished(true);
                if (animate) {
    ...
    public DragItemRecyclerView addColumnList(final DragItemAdapter adapter, final View header, final View footer, boolean hasFixedItemSize) {
        ...
        SmartRefreshLayout srl = (SmartRefreshLayout) LayoutInflater
                .from(getContext())
                .inflate(R.layout.smart_refresh_layout3, this, false);
        final DragItemRecyclerView recyclerView = srl.findViewById(R.id.drag_item_recycler_view);
        recyclerView.setId(getColumnCount());
        recyclerView.setHorizontalScrollBarEnabled(false);
        recyclerView.setVerticalScrollBarEnabled(false);
        recyclerView.setMotionEventSplittingEnabled(false);
        recyclerView.setDragItem(mDragItem);
        LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(mColumnWidth, LayoutParams.MATCH_PARENT);
        ...
        layout.setTag("layout" + mColumnLayout.getChildCount());
        return recyclerView;
    }
