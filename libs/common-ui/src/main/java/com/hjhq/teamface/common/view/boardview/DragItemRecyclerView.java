...
public class DragItemRecyclerView extends RecyclerView implements AutoScroller.AutoScrollListener {
    ...
    private boolean isTaskBoard = false;
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                float diffX = Math.abs(event.getX() - mStartX);
                float diffY = Math.abs(event.getY() - mStartY);
                float diffY2 = event.getY() - mStartY;
                LinearLayoutManager layoutManager = (LinearLayoutManager) getLayoutManager();
                int position1 = layoutManager.findFirstCompletelyVisibleItemPosition();
                int position2 = layoutManager.findLastCompletelyVisibleItemPosition();
                if (diffX > diffY) {
                ...
                if (isTaskBoard) {
                    if (diffY > 1) {
                        if (diffY2 > 0 && position1 == 0) {
                            //向下
                            parent5.requestDisallowInterceptTouchEvent(false);
                            return false;
                        }
                        if (diffY2 < 0 && position2 < ((DragItemAdapter) getAdapter()).getItemCount()) {
                            //向上
                            parent5.requestDisallowInterceptTouchEvent(false);
                            return false;
                        }
                        if ((position1 > 0 || position2 < ((DragItemAdapter) getAdapter()).getItemCount())) {
                ...
                Log.e("XXXX", "==" + diffX + "--------YYYY" + "===" + diffY);
                if (diffY > 1 && diffY > diffX
                        && (position1 > 0 || position2 < ((DragItemAdapter) getAdapter()).getItemCount())) {
